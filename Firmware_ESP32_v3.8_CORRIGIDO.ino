 ...
 * Versão: v3.8 CORRIGIDO (20/10/2025)
 *
 * 🔧 Correções desta versão:
 * - ✅ MQTT_PORT alterado para 8884 (WebSocket Secure)
 * - ✅ Adicionado espClient.setHandshakeTimeout(30) para TLS
 * - ✅ Buffer MQTT aumentado para 1024 bytes
 * - ✅ Lógica de modo de emergência corrigida para boot inicial
 * - ✅ delay(10) removido do loop principal
 * - ✅ Validação adicionada em publishRelayStatus()
 * - ✅ Mantém todas as funcionalidades anteriores (Bluetooth, WiFi Config, etc.)
 *
 *****************************************************************************************/
...
#define MQTT_PORT 8884  // ✅ CORRIGIDO: WebSocket Secure (era 8883)
#define MQTT_USERNAME "esp32-user"
#define MQTT_PASSWORD "HydroSmart123"
#define MQTT_CLIENT_ID "ESP32_Actuator_v3_8_CORRIGIDO"
...
const unsigned long INITIAL_BOOT_TIMEOUT = 120000UL;  // ✅ NOVO: Timeout para primeira conexão
...
unsigned long bootTime = 0;  // ✅ NOVO: Tempo de boot
...
void publishRelayStatus(bool retained = true) {
  ...
  if (!mqttClient.publish(TOPIC_RELAY_STATUS, out.c_str(), retained)) {
    if (!enqueueOutgoing(TOPIC_RELAY_STATUS, out)) {
      logMessage("WARN", "Falha ao publicar relay status e fila cheia");
    }
  }
}
...
void registerWatchdogs() {
  ...
  logMessage("INFO", "Task WDT reconfigurado (60s)");
}
...
void setup() {
  ...
  bootTime = millis();  // ✅ Registrar tempo de boot
  ...
  // ✅ CORRIGIDO: Handshake timeout adicionado
  espClient.setInsecure();
  espClient.setHandshakeTimeout(30);
  
  mqttClient.setServer(MQTT_BROKER, MQTT_PORT);
  mqttClient.setCallback(mqttCallback);
  mqttClient.setBufferSize(1024);  // ✅ CORRIGIDO: Buffer aumentado
  ...
  publishRelayStatus(true);
  logMessage("INFO", "Sistema inicializado - v3.8 CORRIGIDO");
}
...
void loop() {
  ...
  // ✅ CORRIGIDO: Lógica de emergência aprimorada
  if (WiFi.status() == WL_CONNECTED) {
    ...
    // ✅ CORRIGIDO: Modo de emergência também ativa se nunca conectou após timeout inicial
    if ((firstConnectionEstablished && (now - lastOnlineTimestamp > EMERGENCY_MODE_TIMEOUT)) ||
        (!firstConnectionEstablished && (now - bootTime > INITIAL_BOOT_TIMEOUT))) {
      if (!emergencyMode) {
        emergencyMode = true;
        logMessage("WARN", "Conexão perdida ou não estabelecida. Ativando modo de emergência.");
  ...
  // ✅ CORRIGIDO: delay(10) removido para melhor responsividade
}
