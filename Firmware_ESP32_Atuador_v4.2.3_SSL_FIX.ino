/*
 * AquaSys Nexus - Actuator Module v4.2.3-SSL-FIX
 * ==============================================
 * ✅ CORREÇÕES v4.2.3:
 * - SSL_INSECURE_MODE habilitado temporariamente para debug
 * - Logging detalhado de SSL (insecure vs secure mode)
 * - Verificação rigorosa de NTP antes de autenticação SSL
 * - Melhor tratamento de erros SSL
 * 
 * Correções anteriores (v4.2.0):
 * - NTP sincroniza ANTES da autenticação SSL
 * - Aguarda sincronização NTP antes de validar certificados
 */
...
void setup() {
  ...
  initWatchdog();
  logMessage(LOG_INFO, "=== AquaSys Actuator v4.2.3-STABLE ===");
...
void loadConfig() {
  ...
  for (int i = 0; i < 8; i++) {
    ...
    relayConfigs[i].mode = (RelayMode)preferences.getInt((prefix + "mode").c_str(), MODE_MANUAL_OFF);
    ...
    relayConfigs[i].timer_on_duration = preferences.getULong((prefix + "t_on").c_str(), 60000);
    relayConfigs[i].timer_off_duration = preferences.getULong((prefix + "t_off").c_str(), 300000);
  }
...
void saveRelayConfig(int index) {
  ...
  preferences.putULong((prefix + "t_on").c_str(), relayConfigs[index].timer_on_duration);
  preferences.putULong((prefix + "t_off").c_str(), relayConfigs[index].timer_off_duration);
...
bool isValidSensorData(const SensorData& data) {
  return data.ph > 0 && data.ph < 14 &&
         data.ec >= 0 && data.ec < 10 &&
         data.temperature > -10 && data.temperature < 50 &&
         data.humidity >= 0 && data.humidity <= 100 &&
         data.water_temp > 0 && data.water_temp < 40;
}
...
void updateRelays() {
  for (int i = 0; i < 8; i++) {
    ...
    } else if (relayConfigs[i].mode == MODE_TIMER) {
      unsigned long elapsed = millis() - relayConfigs[i].last_timer_change;
      unsigned long threshold = relayConfigs[i].timer_state ?
                                relayConfigs[i].timer_on_duration :
                                relayConfigs[i].timer_off_duration;
      
      if (elapsed >= threshold) {
        relayConfigs[i].timer_state = !relayConfigs[i].timer_state;
        relayConfigs[i].last_timer_change = millis();
        updateRelay(i, relayConfigs[i].timer_state, "timer");
      }
    }
  }
}
    ...
    switch (relayConfigs[i].mode) {
      case MODE_AUTO_PH_DOWN:
        if (currentSensorData.ph > relayConfigs[i].ph_max &&
            millis() - lastPhAction > PH_COOLDOWN) {
          shouldActivate = true;
          reason = "ph_down";
          lastPhAction = millis();
        }
      ...
      case MODE_AUTO_EC_UP:
        if (currentSensorData.ec < relayConfigs[i].ec_min &&
            millis() - lastEcAction > EC_COOLDOWN) {
          shouldActivate = true;
          reason = "ec_up";
          lastEcAction = millis();
        }
      ...
      case MODE_AUTO_TEMP_COOL:
        shouldActivate = currentSensorData.temperature > relayConfigs[i].temp_max;
        reason = "temp_cool";
        break;
        
      case MODE_AUTO_TEMP_HEAT:
        shouldActivate = currentSensorData.temperature < relayConfigs[i].temp_min;
        reason = "temp_heat";
        break;
        
      case MODE_AUTO_HUMIDITY:
        shouldActivate = currentSensorData.humidity < relayConfigs[i].humidity_min;
        reason = "humidity";
        break;
    }
    
    if (shouldActivate && !relayConfigs[i].state) {
      updateRelay(i, true, reason);
    } else if (!shouldActivate && relayConfigs[i].state &&
               relayConfigs[i].mode >= MODE_AUTO_PH_DOWN) {
      updateRelay(i, false, reason + "_off");
    }
  }
}
...
// ==================== AUTENTICAÇÃO ====================
// ✅ CORREÇÃO v4.2.3: Logging detalhado de SSL
bool authenticateDevice() {
  if (!wifiConnected) return false;

  logMessage(LOG_INFO, "Autenticando dispositivo...");

  // ✅ Configurar SSL com logging detalhado
  if (SSL_INSECURE_MODE) {
  ...
  doc["firmware_version"] = "4.2.3-STABLE";
