/*
  AquaSys Sensor Module v2.4 COMPLETO
...
// ----------------------------- VERSÃO / DEVICE -----------------------------------------
#define FIRMWARE_VERSION "2.4-COMPLETO"
  ...
  if (topicStr == "aquasys/sensors/command") {
    ...
    if (!error) {
      if (doc.containsKey("calibrate")) {
        ...
        logInfo("Comando de calibração recebido");
      }
    }
  }
}
...
void handleButtons() {
  ...
  // Botão SELECT - reservado para calibração
  if (digitalRead(BUTTON_SELECT) == LOW && (now - lastDebounce[2]) > debounceDelay) {
    lastDebounce[2] = now;
    Serial.println("[BTN] SELECT - Função de calibração (não implementada)");
  }
  
  // Botão BACK - reservado para menu
  if (digitalRead(BUTTON_BACK) == LOW && (now - lastDebounce[3]) > debounceDelay) {
    lastDebounce[3] = now;
    Serial.println("[BTN] BACK - Função de menu (não implementada)");
  }
}
...
void setup() {
  ...
  Serial.println();
  Serial.println("[INFO] ═══════════════════════════════════════");
  Serial.println("[INFO] AquaSys Sensor v2.4 COMPLETO");
  Serial.println("[INFO] ═══════════════════════════════════════");
  ...
  logInfo("Sistema inicializado - v2.4 COMPLETO");
