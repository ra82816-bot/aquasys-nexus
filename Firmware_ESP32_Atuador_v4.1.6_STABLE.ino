/*
 * AquaSys Nexus - Actuator Module v4.1.6-STABLE
 * =============================================
 * Correções críticas de Watchdog Timer (WDT):
 * ✅ WDT resetado antes de operações bloqueantes no setup()
 * ✅ Lazy BLE initialization (apenas quando necessário)
 * ✅ loadConfig() otimizado (80+ → 16 leituras NVS)
 * ✅ Backoff exponencial MQTT
 * ✅ Autenticação TLS segura
...
const unsigned long PH_COOLDOWN = 300000;  // 5 min
const unsigned long EC_COOLDOWN = 600000;  // 10 min
unsigned long lastPhAction = 0;
unsigned long lastEcAction = 0;

// Dados de Sensores
SensorData currentSensorData = {0, 0, 0, 0, 0, false, 0};
const unsigned long SENSOR_DATA_TIMEOUT = 180000; // 3 min

// Histórico pH 24h
float ph_history[24] = {0};
int ph_history_index = 0;
unsigned long last_ph_log = 0;
const unsigned long PH_LOG_INTERVAL = 3600000; // 1 hora
...
// BLE Client
bool bleInitialized = false;  // ✅ NOVO: Flag para lazy initialization
bool bleClientConnected = false;
String connectedSensorAddress = "";
unsigned long lastBleDataReceived = 0;
unsigned long lastBleScan = 0;
const unsigned long BLE_SCAN_INTERVAL = 30000; // 30s
...
void setup() {
  ...
  // ✅ CORREÇÃO P0: Inicializar WDT PRIMEIRO
  initWatchdog();
  
  logMessage(LOG_INFO, "=== AquaSys Actuator v4.1.6-STABLE ===");
  ...
  // ✅ CORREÇÃO P1: loadConfig() otimizado
  loadConfig();
  
  // ✅ CORREÇÃO P1: BLE lazy initialization (removido daqui)
  // setupBLE(); será chamado apenas quando necessário no loop()
  
  // WiFi
  esp_task_wdt_reset();  // ✅ CORREÇÃO P0: Reset antes de operação bloqueante
  setupWiFi();
  
  // Autenticação
  if (wifiConnected && !apMode) {
    esp_task_wdt_reset();  // ✅ CORREÇÃO P0: Reset antes de autenticação
  ...
  // NTP
  if (wifiConnected && !apMode) {
    esp_task_wdt_reset();  // ✅ CORREÇÃO P0: Reset antes de NTP
    setupNTP();
  }
  
  // MQTT
  if (authCompleted) {
    esp_task_wdt_reset();  // ✅ CORREÇÃO P0: Reset antes de MQTT
    setupMQTT();
  }
...
void loop() {
  ...
  // Modo AP
  if (apMode) {
    esp_task_wdt_reset();  // ✅ CORREÇÃO: Reset no loop AP
    server.handleClient();
    delay(10);  // ✅ CORREÇÃO: Small delay para evitar WDT
    return;
  }
  ...
  // ✅ CORREÇÃO P1: Lazy BLE initialization
  // Ativar BLE Client apenas quando necessário (após 3 min sem MQTT)
  if (!mqttConnected && !bleActivated && 
      (millis() - currentSensorData.timestamp > SENSOR_DATA_TIMEOUT)) {
    logMessage(LOG_WARN, "MQTT offline > 3min - ativando BLE Client");
    bleActivated = true;
    
    // Inicializar BLE agora (lazy initialization)
    if (!bleInitialized) {
      esp_task_wdt_reset();  // ✅ Reset antes de operação bloqueante
      setupBLE();
      bleInitialized = true;
    }
  }
  
  // BLE Client scan periódico
...
void initWatchdog() {
  ...
  logMessage(LOG_INFO, "Watchdog inicializado (" + String(WDT_TIMEOUT) + "s timeout)");
}
...
// ==================== PERSISTÊNCIA ====================
// ✅ CORREÇÃO P1: loadConfig() otimizado - 80+ → ~16 leituras NVS
void loadConfig() {
  preferences.begin("hydrosmart", false);
  
  // Carregar apenas configurações personalizadas
  // Valores padrão já foram definidos em setupRelays()
  for (int i = 0; i < 8; i++) {
    String prefix = "r" + String(i) + "_";
    
    // Apenas carrega se existir valor salvo (não usa getFloat com default)
...
// ==================== WIFI ====================
void setupWiFi() {
  // Carregar credenciais do NVS se vazias
  ...
  int attempts = 0;
  while (WiFi.status() != WL_CONNECTED && attempts < 10) {  // ✅ CORREÇÃO P2: 10 → 5s
    delay(500);
    Serial.print(".");
    attempts++;
    
    // ✅ CORREÇÃO P2: Reset WDT a cada 5 tentativas
    if (attempts % 5 == 0) {
      esp_task_wdt_reset();
    }
  }
...
void startAPMode() {
  apMode = true;
  apSSID = "AquaSys-" + deviceUUID.substring(4, 16);  // Usar últimos 12 chars do UUID
...
bool authenticateDevice() {
  ...
  if (httpCode == 200) {
    ...
    if (!error) {
      if (responseDoc["authenticated"] == true) {
...
void reconnectMQTT() {
  ...
  // Calcular backoff exponencial: 5s, 10s, 20s, 40s, 60s (max)
  unsigned long backoff = MQTT_RECONNECT_INTERVAL * (1 << mqttReconnectAttempts);
  if (backoff > 60000) backoff = 60000;
  
  if (millis() - lastMqttReconnect < backoff) {
    return; // Ainda não é hora de tentar
  }
  ...
  if (mqttClient.connect(clientId.c_str(), mqttUsername.c_str(), mqttPassword.c_str())) {
    ...
    logMessage(LOG_WARN, "MQTT falhou (rc=" + String(mqttClient.state()) + 
               "). Próxima tentativa em " + String((backoff * 2) / 1000) + "s");
  }
}
...
void publishHeartbeat() {
  ...
  doc["device_uuid"] = deviceUUID;
  doc["device_type"] = "actuator";
  doc["firmware_version"] = "4.1.6-STABLE";
  doc["timestamp"] = getTimestamp();
...
void setupBLE() {
  ...
  logMessage(LOG_INFO, "BLE Client pronto");
}
