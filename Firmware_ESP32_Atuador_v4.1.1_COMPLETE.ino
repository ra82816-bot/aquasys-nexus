/*
 * AquaSys Nexus - Actuator Module v4.1.1-COMPLETE
 * ================================================
 * Firmware completo com TODAS as funcionalidades:
 * - 9 Modos de Relé (Manual, Auto pH/EC/Temp/Humidity, Timer)
 * - Histórico pH 24h
 * - Debounce e Cooldowns
 * - Auto-reboot após falhas de autenticação
 * - BLE Client para fallback local
 * - TLS 1.3 seguro
 * - Watchdog ESP-IDF 5.x
 * - Backoff exponencial MQTT
 * - Pinos corretos: {23, 5, 4, 13, 22, 21, 14, 12}
 */
...
void setup() {
  ...
  logMessage(LOG_INFO, "=== AquaSys Actuator v4.1.1-COMPLETE ===");
  ...
  // BLE (preparar, mas não ativar ainda)
  setupBLE();
...
void loop() {
  ...
  // Ativar BLE Client após 3 min sem MQTT
...
void logMessage(LogLevel level, String message) {
  ...
  Serial.println("[" + getTimestamp() + "] [" + levelStr + "] " + message);
...
void startAPMode() {
  apMode = true;
  apSSID = "AquaSys-" + deviceUUID.substring(0, 8);
...
void reconnectMQTT() {
  ...
  if (mqttReconnectAttempts >= MAX_MQTT_RECONNECT_ATTEMPTS) {
    if (millis() - lastMqttReconnect > 300000) {
  ...
  logMessage(LOG_INFO, "Conectando ao MQTT... (tentativa " + 
  ...
  if (mqttClient.connect(clientId.c_str(), mqttUsername.c_str(), mqttPassword.c_str())) {
    ...
    // Backoff exponencial desde primeira tentativa
    unsigned long backoff = min(300000UL, (unsigned long)(5000UL * pow(2.0, mqttReconnectAttempts - 1)));
    lastMqttReconnect = millis();
    
    logMessage(LOG_WARN, "MQTT falhou (rc=" + String(mqttClient.state()) + 
               "). Próxima tentativa em " + String(backoff / 1000) + "s");
  }
}
...
void publishHeartbeat() {
  ...
  doc["device_uuid"] = deviceUUID;
  doc["device_type"] = "actuator";
  doc["firmware_version"] = "4.1.1-COMPLETE";
  doc["timestamp"] = getTimestamp();
...
void readSensorDataBLE() {
  ...
  if (pBLEClient->isConnected()) {
    ...
    logMessage(LOG_DEBUG, "BLE data recebido: " + data);
